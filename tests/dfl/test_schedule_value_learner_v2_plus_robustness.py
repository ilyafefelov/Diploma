...
def test_v2_plus_robustness_generates_four_latest_first_windows() -> None:
    library = _candidate_library_104(v2_plus_window_regrets=[70.0, 70.0, 70.0, 85.0])

    result = _build_robustness(library)
    evidence = validate_dfl_schedule_value_learner_v2_plus_robustness_evidence(result)
    ...
    source = result.filter(
        pl.col("source_model_name") == "nbeatsx_official_global_panel_v1"
    ).sort("window_index")
    assert source["window_index"].to_list() == [1, 2, 3, 4]
    assert source["validation_tenant_anchor_count"].to_list() == [90, 90, 90, 90]
    assert source["minimum_prior_anchor_count_before_window"].to_list() == [86, 68, 50, 32]
    assert source["validation_start_anchor_timestamp"].to_list() == [
...
def test_v2_plus_robustness_selection_uses_prior_not_validation_actuals() -> None:
    library = _candidate_library_104(v2_plus_window_regrets=[70.0, 70.0, 70.0, 70.0])
    mutated = _candidate_library_104(v2_plus_window_regrets=[700.0, 70.0, 70.0, 70.0])

    base = _build_robustness(library)
    after_mutation = _build_robustness(mutated)

    latest_base = _latest_window(base)
    latest_mutated = _latest_window(after_mutation)
    assert latest_base["fallback_to_v2_by_tenant"] == latest_mutated[
        "fallback_to_v2_by_tenant"
    ]
    assert latest_base["selected_family_counts"] == latest_mutated[
        "selected_family_counts"
    ]
    assert latest_base["selected_mean_regret_uah"] == 70.0
    assert latest_mutated["selected_mean_regret_uah"] == 700.0
    assert latest_base["v2_plus_window_passed"] is True
    assert latest_mutated["v2_plus_window_passed"] is False


def test_v2_plus_robustness_gate_requires_three_strict_and_v2_passes() -> None:
    robust_library = _candidate_library_104(
        v2_plus_window_regrets=[70.0, 70.0, 70.0, 85.0]
    )
    weak_library = _candidate_library_104(
        v2_plus_window_regrets=[70.0, 85.0, 85.0, 85.0]
    )

    robust = _build_robustness(robust_library)
    weak = _build_robustness(weak_library)
    robust_gate = evaluate_dfl_schedule_value_learner_v2_plus_robustness_gate(robust)
    weak_gate = evaluate_dfl_schedule_value_learner_v2_plus_robustness_gate(weak)

    assert robust_gate.passed is False
    assert robust_gate.decision == "v2_plus_robust_research_challenger"
    assert robust_gate.metrics["robust_source_model_names"] == list(SOURCE_MODELS)
    assert set(robust["robust_research_challenger"]) == {True}
    assert weak_gate.passed is False
    assert weak_gate.decision == "diagnostic_pass_not_robust"
    ...
    [
        (
            lambda: _candidate_library_104(
                v2_plus_window_regrets=[70.0, 70.0, 70.0, 70.0],
...
def test_v2_plus_robustness_blocks_invalid_evidence(
...
def _build_robustness(frame: pl.DataFrame) -> pl.DataFrame:
    return build_dfl_schedule_value_learner_v2_plus_robustness_frame(
        frame,
        tenant_ids=TENANTS,
...
def _latest_window(frame: pl.DataFrame) -> dict[str, object]:
    return frame.filter(
        (pl.col("source_model_name") == "nbeatsx_official_global_panel_v1")
...
def _candidate_library_104(
    *,
    v2_plus_window_regrets: list[float],
    anchor_count: int = 104,
    data_quality_tier: str = "thesis_grade",
    safety_violation_count: int = 0,
    not_market_execution: bool = True,
) -> pl.DataFrame:
    ...
    for tenant_id in TENANTS:
        for source_model_name in SOURCE_MODELS:
            for anchor_index in range(anchor_count):
                ...
                v2_plus_regret = (
                    v2_plus_window_regrets[window_for_anchor[anchor_index] - 1]
                ...
                rows.extend(
                    [
                        _candidate_row(
                            ...
                            not_market_execution=not_market_execution,
                        ),
                        _candidate_row(
                            tenant_id=tenant_id,
                            source_model_name=source_model_name,
