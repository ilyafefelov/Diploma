...
from smart_arbitrage.dfl.promotion_gate import evaluate_offline_dfl_action_target_promotion_gate
...
V4_MODEL = action_target_model_name(SOURCE_MODEL)
FIRST_ANCHOR = datetime(2026, 1, 1, 23)
GENERATED_AT = datetime(2026, 5, 7, 10)


def test_action_target_panel_keeps_selected_parameters_fixed_when_final_actuals_change() -> None:
    panel = _panel_frame(anchor_count_per_tenant=20, final_validation_anchor_count=2)
    base_benchmark = _benchmark_frame(
        anchor_count_per_tenant=20,
        final_validation_anchor_count=2,
        final_actual_prices=(1000.0, 5000.0),
    )
    mutated_benchmark = _benchmark_frame(
        anchor_count_per_tenant=20,
        final_validation_anchor_count=2,
        final_actual_prices=(5000.0, 1000.0),
    )
    base_decision_panel = _decision_panel(base_benchmark, panel, final_validation_anchor_count=2)
    mutated_decision_panel = _decision_panel(mutated_benchmark, panel, final_validation_anchor_count=2)

    base = build_offline_dfl_action_target_panel_frame(
        base_benchmark,
        panel,
        base_decision_panel,
        ...
        max_train_anchors_per_tenant=4,
        inner_validation_fraction=0.5,
        charge_hour_count_grid=(1,),
        discharge_hour_count_grid=(1,),
        action_spread_grid_uah_mwh=(1000.0,),
        include_panel_v2_bias_options=(False,),
        include_decision_v3_correction_options=(False,),
    )
    mutated = build_offline_dfl_action_target_panel_frame(
        mutated_benchmark,
        panel,
        mutated_decision_panel,
        tenant_ids=CANONICAL_TENANTS[:1],
        forecast_model_names=(SOURCE_MODEL,),
        final_validation_anchor_count_per_tenant=2,
        max_train_anchors_per_tenant=4,
    ...
    assert base_row["charge_hour_count"] == mutated_row["charge_hour_count"]
    assert base_row["discharge_hour_count"] == mutated_row["discharge_hour_count"]
    assert base_row["action_spread_uah_mwh"] == mutated_row["action_spread_uah_mwh"]
    assert base_row["include_panel_v2_bias"] == mutated_row["include_panel_v2_bias"]
    assert base_row["include_decision_v3_correction"] == mutated_row["include_decision_v3_correction"]
    assert base_row["inner_selection_mean_regret_uah"] == mutated_row["inner_selection_mean_regret_uah"]


def test_action_target_strict_benchmark_emits_final_holdout_rows_and_applies_formula() -> None:
    ...
    decision_panel = _decision_panel(benchmark, panel, final_validation_anchor_count=18)
    action_panel = build_offline_dfl_action_target_panel_frame(
        benchmark,
        panel,
        decision_panel,
    ...
    result = build_offline_dfl_action_target_strict_lp_benchmark_frame(
        benchmark,
        panel,
        decision_panel,
        action_panel,
    ...
    assert result.height == 5 * 18 * 5
    assert result.select("tenant_id").n_unique() == 5
    assert result.filter(pl.col("forecast_model_name") == V4_MODEL).height == 90
    ...
    v4_row = result.filter(pl.col("forecast_model_name") == V4_MODEL).row(0, named=True)
    payload = v4_row["evaluation_payload"]
    assert payload["source_forecast_model_name"] == SOURCE_MODEL
    assert payload["action_target_v4_forecast_model_name"] == V4_MODEL
    assert payload["action_target_v4_charge_hour_count"] == 1
    assert payload["action_target_v4_discharge_hour_count"] == 1
    assert payload["action_target_v4_action_spread_uah_mwh"] == 1000.0
    assert payload["action_target_v4_include_panel_v2_bias"] is False
    assert payload["action_target_v4_include_decision_v3_correction"] is False
    assert [point["forecast_price_uah_mwh"] for point in payload["horizon"]] == [5500.0, 500.0]


def test_action_target_panel_fails_for_missing_or_invalid_inputs() -> None:
    benchmark = _benchmark_frame(anchor_count_per_tenant=20)
    panel = _panel_frame(anchor_count_per_tenant=20, final_validation_anchor_count=18)
    decision_panel = _decision_panel(benchmark, panel, final_validation_anchor_count=18)

    with pytest.raises(ValueError, match="missing decision-target panel row"):
        build_offline_dfl_action_target_panel_frame(
            benchmark,
            panel,
            decision_panel.filter(pl.col("tenant_id") != CANONICAL_TENANTS[0]),
            tenant_ids=CANONICAL_TENANTS[:1],
            forecast_model_names=(SOURCE_MODEL,),
            final_validation_anchor_count_per_tenant=18,
        )

    invalid_decision_panel = decision_panel.with_columns(
        pl.Series("panel_v2_horizon_biases_uah_mwh", [[-200.0] for _ in CANONICAL_TENANTS])
    )
    with pytest.raises(ValueError, match="bias length"):
        build_offline_dfl_action_target_panel_frame(
            benchmark,
            panel,
            invalid_decision_panel,
            tenant_ids=CANONICAL_TENANTS[:1],
            forecast_model_names=(SOURCE_MODEL,),
            final_validation_anchor_count_per_tenant=18,
        )
...
def test_action_target_promotion_gate_passes_only_when_v4_beats_strict_control() -> None:
    result = evaluate_offline_dfl_action_target_promotion_gate(_strict_gate_frame(v4_regret_uah=90.0))
...
def test_action_target_promotion_gate_blocks_under_90_or_non_thesis_evidence() -> None:
    under_90 = evaluate_offline_dfl_action_target_promotion_gate(
        _strict_gate_frame(anchor_count_per_tenant=17, v4_regret_uah=90.0)
    )
    non_thesis = evaluate_offline_dfl_action_target_promotion_gate(
        _strict_gate_frame(v4_regret_uah=90.0, data_quality_tier="demo_grade")
    )
...
def test_action_target_promotion_gate_blocks_median_degradation_and_weak_mean() -> None:
    worse_median = evaluate_offline_dfl_action_target_promotion_gate(
        _strict_gate_frame(v4_regrets=([0.0] * 20) + ([101.0] * 70))
    )
    weak_mean = evaluate_offline_dfl_action_target_promotion_gate(
        _strict_gate_frame(v4_regret_uah=97.0)
    )
...
def _decision_panel(
    ...
    *,
    ...
    return build_offline_dfl_decision_target_panel_frame(
        benchmark,
        panel,
        tenant_ids=CANONICAL_TENANTS,
        forecast_model_names=(SOURCE_MODEL,),
        final_validation_anchor_count_per_tenant=final_validation_anchor_count,
        max_train_anchors_per_tenant=2,
        inner_validation_fraction=0.5,
...
def _strict_gate_frame(
    *,
    ...
    v4_regret_uah: float | None = None,
    v4_regrets: list[float] | None = None,
    ...
    resolved_v4_regrets = v4_regrets or [float(v4_regret_uah if v4_regret_uah is not None else 90.0)] * len(
        tenant_anchors
    )
    for index, (tenant_id, anchor) in enumerate(tenant_anchors):
        ...
        rows.append(_strict_gate_row(tenant_id=tenant_id, anchor=anchor, model_name=V3_MODEL, regret=115.0))
        rows.append(
            _strict_gate_row(
                tenant_id=tenant_id,
                anchor=anchor,
                model_name=V4_MODEL,
                regret=resolved_v4_regrets[index],
