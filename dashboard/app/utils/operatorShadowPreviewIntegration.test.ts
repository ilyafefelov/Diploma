...
describe('operator shadow preview integration', () => {
  ...
  it('keeps strategy comparison chart anchored to the best-valid recommendation', () => {
    ...
    expect(page).toContain(':operator-recommendation="selectedVisibleOperatorRecommendation"')
    expect(page).toContain(':best-valid-recommendation="bestValidStrategyComparisonRecommendation"')
  })
  ...
  it('presents one schedule source switch and keeps strict baseline as context', () => {
    ...
    expect(header).toContain('HF value-aligned demo scenarios')
    expect(header).toContain('select:hf-demo-scenario')
    ...
    expect(previewSources).toContain('HF live safe-switch shadow')
    expect(previewSources).toContain('HF live safe-switch value-aligned shadow')
