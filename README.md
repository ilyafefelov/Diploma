...
- Current model-improvement branch: Regret-Surrogate DFL v1 materialized a learning-limit audit before another DT attempt. The latest final holdout has oracle-switch upside (`174.77` -> `161.38` UAH mean regret, `7.66%`) but only `8 / 90` final rows have a better candidate; the conservative surrogate selected `0 / 90` non-V2+ rows, matched V2+ at `174.77` / `67.30` UAH, and produced `0 / 4` rolling challenger windows. V2+ remains the thesis headline while the next research branch needs stronger teacher labels, candidates, or context.
