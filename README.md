...
- Current model-improvement branch: Candidate-Value DFL v3 expands the feasible schedule library around audited failure modes and trains a learned candidate-level value scorer from the V3 label panel, with V2+ fallback. The 2026-05-18 materialized run passed label-panel, strict-benchmark, and failure-audit checks but matched V2+ exactly: calibrated mean regret `174.77` UAH, raw mean regret `193.36` UAH, and `0.00%` improvement versus V2+. The V4 plateau-breaker materialization also passed evidence checks and separated candidate weakness from fallback conservatism, but it again selected V2+ for all tenant/source rows. V2+ remains the thesis headline.
