...
- Current model-improvement branch: Candidate-Value DFL v3 expanded the feasible schedule library around audited failure modes and trained a learned candidate-level value scorer from the V3 label panel, with V2+ fallback. The V3 and V4 materialized runs passed evidence checks but selected V2+ for all tenant/source rows. The Point-in-Time Context Repair + Candidate-Value DFL V5 run also passed its evidence check but selected V2+ for all tenant/source rows: calibrated mean regret stayed `174.77` UAH, raw mean regret stayed `193.36` UAH, and improvement versus V2+ stayed `0.00%`. V2+ remains the thesis headline.
