...
- Current model-improvement branch: Candidate-Value DFL v3 expands the feasible schedule library around audited failure modes and scores full candidate schedules with V2+ fallback. The first materialized run passed the evidence check but matched V2+ exactly: calibrated mean regret `174.77` UAH, raw mean regret `193.36` UAH, and `0.00%` improvement versus V2+. V2+ remains the thesis headline.
