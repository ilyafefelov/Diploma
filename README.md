...
- Current model-improvement branch: Candidate-Value DFL v3 expanded the feasible schedule library around audited failure modes and trained a learned candidate-level value scorer from the V3 label panel, with V2+ fallback. The V3 and V4 materialized runs passed evidence checks but selected V2+ for all tenant/source rows, so V2+ remains the thesis headline. The next implemented gate is Point-in-Time Context Repair + Candidate-Value DFL V5: it turns V4 weather/load, calendar/event, and publication-time gaps into prior-only Ukrainian `selector_feature_*` context features before rerunning the unchanged V2+ fallback gate.
