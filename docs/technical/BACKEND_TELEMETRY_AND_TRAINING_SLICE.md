...
docker compose exec -T dagster-webserver uv run dagster asset materialize -m smart_arbitrage.defs --select forecast_strategy_comparison_frame
...
Invoke-RestMethod "http://localhost:8001/dashboard/forecast-strategy-comparison?tenant_id=client_001_kyiv_mall"
```
...
- `forecast_strategy_evaluations` contains one latest Gold comparison row per forecast candidate and tenant.
...
- Add Gold forecast-strategy cards from `/dashboard/forecast-strategy-comparison`: decision value, oracle value, regret, degradation penalty, throughput, and starting SOC source.
