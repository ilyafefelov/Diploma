...
forecast/read-model context
  ...
  -> feasible hourly recommendation schedule
  -> operator review UI
```
...
| `market_venue` | `DAM` |
| `interval_minutes` | `60` |
| `anchor_timestamp` | As-of anchor used by the visible LP preview |
...
| `target_delivery_window_start` / `target_delivery_window_end` | Delivery window covered by the visible hourly schedule |
...
The schedule dock may show charge/discharge/hold schedule intervals, but it must frame them as preview/review intervals, not dispatch automation.
