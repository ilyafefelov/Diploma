...
- ✅ Gold research layer — forecast diagnostics, value-aware ensemble, calibrated horizon-aware ensemble gate, DFL training table, scalar and horizon-aware regret-weighted TFT/NBEATSx calibration.
- ✅ Strict LP/oracle re-evaluation — calibrated forecasts checked against same Level 1 simulator.
- ✅ Backend read model — `/dashboard/calibrated-ensemble-benchmark` exposes the latest calibrated selector rows for dashboard redesign without touching dashboard UI.
