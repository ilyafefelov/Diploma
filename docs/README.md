...
- ✅ DFL schedule/value offline promotion gate — `dfl_schedule_value_production_gate_frame` sets `production_promote=true` for both source-specific Schedule/Value Learner V2 challengers in offline/read-model evidence only, persists the latest rows to `dfl_schedule_value_production_gate_rows`, and exposes `/dashboard/dfl-schedule-value-production-gate` while `market_execution_enabled=false` and `strict_similar_day` remains fallback. See [DFL_SCHEDULE_VALUE_PRODUCTION_GATE.md](technical/DFL_SCHEDULE_VALUE_PRODUCTION_GATE.md).
- ✅ Official schedule/value promotion path — official NBEATSx/TFT rolling forecasts now have an additive schedule-candidate library, Schedule/Value Learner V2, robustness, and offline promotion-gate path; materialized metrics are tracked in [DFL_OFFICIAL_SCHEDULE_VALUE_PROMOTION.md](technical/DFL_OFFICIAL_SCHEDULE_VALUE_PROMOTION.md).
