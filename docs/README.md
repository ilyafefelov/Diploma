...
- ✅ Point-in-time context repair / Candidate-Value DFL V5 gate — `dfl_point_in_time_context_repair_audit_frame`, `dfl_point_in_time_context_feature_panel_frame`, and the V5 context-enriched candidate-value assets convert V4 weather/load, calendar/event, and publication-time gaps into prior-only selector features before the unchanged V2+ fallback gate. See [DFL_POINT_IN_TIME_CONTEXT_REPAIR.md](technical/DFL_POINT_IN_TIME_CONTEXT_REPAIR.md).
