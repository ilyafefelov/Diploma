...
> **Post-defense model-lineage correction (2026-07-12).** The canonical
> `dt_v2_plus` artifact discussed below is a `RandomForestRegressor`, not a
> Decision Transformer. Its training rows exactly mirror the evaluation packet,
> and all four nonfallback profile rows occur on one delivery date. Read every
> `DT/V2+` label for the 168.16 UAH result as a historical artifact identifier,
> not estimator identity or OOS evidence. See
...
DFL-напрям у роботі не відкинуто, але його межу уточнено. Ранні candidate-value DFL v3, residual/DT bridge і regret-aware selector були корисними як negative evidence. DFL v2 matched V2+ на рівні mean regret 174.77 UAH, але не створив replacement improvement, а перший conservative regret-aware run зробив 0 / 90 non-V2+ switches. Історичний `dt_v2_plus` follow-up оптимізує `regret_delta_vs_v2_plus_uah` і abstains до V2+, але фактично реалізований через `RandomForestRegressor`, не DT. Його mean regret 168.1566 UAH, 4 / 90 non-V2+ switches і 86 / 90 abstentions відтворюються, проте training candidates є exact timestamp-shifted copies evaluation candidates, а всі чотири switches припадають на 2026-04-15. Тому це in-packet construction diagnostic, не final-holdout/OOS або positive transformer evidence; V2+ залишається fallback і headline.

Decision Transformer у поточному evidence set треба описувати як окрему research lane. Ранній HF candidate-index / schedule-family classifier з cross-entropy objective оптимізував відтворення teacher labels, а не LP/oracle regret напряму; його 460.30 UAH є mirrored-row pipeline smoke. Пізніший HF value-aligned scorer дійсно завантажує `DecisionTransformerModel`, але використовує його як candidate-scoring backbone з zero actions/rewards/returns-to-go, а 158.71 UAH threshold-selected diagnostic походить із того самого mirrored packet. Random-forest 168.16 UAH належить іншій lineage. Жодний із цих results не є independent OOS evidence, raw BUY/SELL/HOLD controller, dashboard/API default або DT/LAVA promotion. Додатково V13 source-readiness не дозволяє описувати DT/LAVA як готовий training або execution contour.

Кількісне порівняння підходів наведено на рисунку 4.4.

![Рисунок 4.4. Історичне порівняння mean regret; 168.16 UAH bar є random-forest exact-mirror diagnostic, не DT](assets/compact-fig-4-4-method-regret-comparison.png)

Рисунок 4.4. Історичне порівняння mean regret. Позначка `DT/V2+` на збереженому зображенні є застарілою: 168.16 UAH належить random forest exact-mirror diagnostic, а HF 158.71 UAH є окремим mirrored-packet diagnostic.

Рисунок 4.4 зберігається як історичний thesis artifact, але його RF/HF bars не можна ранжувати як independent model results. V2+ залишається confirmed retrospective comparator. Random-forest 168.16 UAH та HF 158.71 UAH походять із mirrored-packet diagnostics і не доводять production replacement, OOS improvement або перевагу transformer architecture.
...
| Random-forest V2+ diagnostic (historical id `dt_v2_plus`) | 168.16 UAH; 4 / 90 profile-row switches, all on one delivery date; exact-mirror packet | Construction diagnostic only; not DT/OOS; V2+ remains comparator |
...
З таблиці 4.3 видно, що подальші candidate-value кроки не дали independently validated replacement of V2+. Random forest знайшов чотири profile-row changes лише в exact-mirror packet і на одній delivery date; це не можна називати безпечними OOS обходами або positive promotion evidence. Для дипломної роботи важливо зберегти ці числа як lineage diagnostic і показати disciplined non-promotion.

Додатковий same-packet threshold diagnostic перевірив `min_predicted_improvement_uah` = 0, 5, 10, 20 і 50. Thresholds 0-20 дали 168.16 UAH, 4 / 90 switches і 86 / 90 abstentions; threshold 50 лишив той самий mean, але скоротив switches до 3 / 90. Це показує лише інваріантність construction packet до цих threshold values. Exact mirroring і single-date support не дозволяють трактувати це як robustness або promotion evidence.
...
Рисунок 4.5 фіксує логіку gate-based evidence. Якщо новий selector не має незалежно перевіреного prior-only сигналу, abstention і fallback до V2+ є правильним рішенням. Same-packet switches можна показувати лише як construction diagnostic; слово "safe" стосується змодельованих guards, а не емпіричної безпеки чи OOS performance. Це зменшує ризик перебільшення claims і формує реалістичний roadmap.
...
| Random-forest V2+ diagnostic (historical id `dt_v2_plus`) | 168.16 UAH проти V2+ 174.77 UAH; exact-mirror packet; 4 / 90 profile-row switches на одній delivery date | Non-independent construction diagnostic; not DT/OOS; no risk inference |
...
Таблиця 4.4 важлива не менше за headline. Вона показує, чому model family не можна просувати через сучасну назву або локально привабливий сигнал. Poland prior-only veto був near-miss, а історичний `dt_v2_plus` RF знайшов вузький same-packet signal, який post-defense audit визнав exact-mirror і single-date. Отже, V2+ не замінюється; roadmap потребує frozen prospective protocol, а не повторного використання цього diagnostic packet.
...
Рисунок 4.7 відділяє working sequence-policy pipeline від deployed controller. Ранній HF apples-to-apples DT classifier дав 460.30 UAH лише як mirrored-row pipeline smoke. Історичний `dt_v2_plus` result 168.16 UAH створено random forest на exact-mirror packet, тому він не є final-holdout або transformer evidence. Додатково V13 не дозволяє DT/LAVA training claims, доки explicit OREE DAM/IDM source/publication evidence for preview залишається blocker.

Причини зупинки learned challengers різні, але логіка однакова. Raw HF DT не промоутиться, бо 460.30 UAH походить із mirrored-row smoke і немає надійного V2+ fallback. Історичний `dt_v2_plus` RF з 168.16 UAH має exact-mirror train/evaluation rows, 4 / 90 profile switches на одній date і 86 / 90 abstentions, тому primary threshold 166.0 UAH не є головною проблемою: немає independent evaluation. HF value-aligned `DecisionTransformerModel`-backbone scorer має 158.71 UAH на тому самому mirrored packet; окремі 20 / 32 nonfallback days належать read-model audit без realized regret. Ці diagnostics не можна зводити в model leaderboard. Stop/abstain і точна lineage є частиною safety case.
...
Цей шлях пояснює, чому результат не зводиться до одного числа. V2 довів корисність schedule/value selection проти strict baseline. V2+ став headline, бо одночасно знизив mean regret, median regret і пройшов rolling-window sensitivity. Shadow lanes після цього мали роль diagnostics. Історичний `dt_v2_plus` RF дав локально нижчий same-packet mean, але exact mirroring і single-date support виключають promotion; future challenger має починатися з нового frozen prospective protocol.
...
Ці числа пояснюють, чому HF value-aligned shadow можна показувати у demo як manually selected read-model source, але не можна називати replacement of V2+ або LP. Порівняно з раннім raw DT classifier, новий HF backbone має fallback/abstention framing, але frozen 158.71 UAH threshold-selected diagnostic все ще використовує mirrored packet. Порівняння з історичним `dt_v2_plus` RF також не є model leaderboard. HF shadow ранжує скінченну бібліотеку candidate schedules, а не розв'язує повний optimization space; окремий 32-day audit показує source/read-model readiness без realized regret. Це корисно для preview, але не є доказом OOS improvement або глобальної оптимальності.
...
Головний результат роботи - V2+ schedule/value selector з mean regret 174.77 UAH, median regret 67.30 UAH, improvement 43.73% проти strict і 15.31% проти V2, а також rolling robustness 4 / 4. Історичний `dt_v2_plus` follow-up фактично використовує random forest: 168.16 UAH, 4 / 90 profile-row switches, усі на одній delivery date, із exact-mirror training/evaluation packet. Це construction diagnostic, а не OOS або transformer evidence. Окремий HF `DecisionTransformerModel`-backbone scorer має 158.71 UAH mirrored-packet diagnostic; його 32-day read-model audit (20 / 32 nonfallback days) не містить realized-regret outcome. Обидві learned-challenger лінії лишаються manual diagnostics; V2+ є confirmed offline schedule-value evidence/comparator, а production market promotion і market execution залишаються false. Підсумкова межа незмінна: DAM/IDM hourly recommendation preview, offline/read-model evidence і no market execution.
