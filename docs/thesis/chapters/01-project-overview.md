...
Для досягнення цієї мети виконано такі завдання: сформовано market/problem framing для українського DAM/IDM preview; побудовано pipeline source snapshots -> normalized panel -> official-row/forecast-scenario context -> strict LP/oracle scoring -> dashboard/read model; визначено метрики regret/value та rolling robustness; перевірено Schedule/Value Learner V2 і V2+ проти strict_similar_day; проаналізовано TFT, Poland lag24 / prior-only veto, історичний `dt_v2_plus` random-forest diagnostic і окремий HF `DecisionTransformerModel`-backbone shadow як дослідницькі/manual preview гілки, але не default/execution гілки; перевірено live shadow-readiness для DAM/IDM latest/today/tomorrow/day+2 без LP у live HF path; зафіксовано V13 source-readiness blockers і прапорець market_execution_enabled=false. Post-defense audit встановив, що RF/HF frozen diagnostics використовують mirrored packet і не є OOS evidence.
