...
Оновлений DFL/DT source refresh додає чіткішу методологічну опору для наступного кроку після forecast-only оцінювання. DFL survey Mandi et al. задає загальну рамку: модель має навчатися на downstream decision quality. Sang et al. роблять цю рамку ближчою до цього диплома, бо прямо розглядають ESS arbitrage і regret/surrogate-regret training. Persak and Anjos підсилюють аргумент, що storage arbitrage є multistage problem, де SOC-path має значення. Perturbed DFL for storage пояснює, чому для LP-шару потрібні surrogate або perturbed losses. Decision Transformer і Hugging Face implementation docs залишаються roadmap-джерелами для offline sequence-modeling candidate, але не є доказом live DT controller у поточному MVP.

Пізніший source capture також додає важливий методологічний guardrail для поточного стану продукту. GIFT-Eval, fev-bench і TFRBench корисні не тому, що їх потрібно негайно інтегрувати як залежності, а тому, що вони підсилюють вимогу до реалістичного temporal evaluation: модель має оцінюватися на доступній на момент рішення інформації, а не на прихованому доступі до майбутніх значень. TSFM leakage evaluation додатково пояснює, чому для time-series foundation models особливо небезпечні latest-batch або benchmark-leakage ефекти. Для цього диплома це прямо підтримує рішення тримати rolling-origin protocol, coverage audit і latest-batch checks як блокуючі умови перед будь-яким DFL/DT claim.

Окремий практичний блок джерел пояснює, чому поточні NBEATSx/TFT або action-classifier candidates можуть програвати `strict_similar_day`, не руйнуючи наукову логіку проєкту. DFKI/NEIS source note про вплив прогнозів на BESS dispatch, Lago et al. про EPF benchmarking і storage-specific DFL джерела показують, що цінність forecast визначається не лише формою прогнозної кривої, а тим, чи приводить вона до кращого feasible schedule. Тому негативні або заблоковані кандидати мають трактуватися як evidence, а не як помилка реалізації: вони показують, що hourly classification або raw forecast improvement ще не є decision-value improvement.

Це уточнення також пояснює роль європейських джерел. PriceFM, THieF, ENTSO-E, OPSD, Nord Pool і Ember задають напрям майбутнього external validation та market-coupling context. Проте вони не мають змішуватися з українським training/evaluation panel без окремого шару licensing, timezone alignment, currency normalization, market-rule mapping і domain-shift analysis. Отже, найближчий коректний шлях залишається UA-first: observed OREE DAM, Open-Meteo, UAH-native regret, frozen LP baseline і тільки після цього DFL/DT research challenger.
