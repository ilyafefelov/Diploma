# Бібліографічна база — citation/index archive

Цей каталог містить citation/source metadata для джерел, на які спирається
дипломна робота. Для public GitHub showcase PDF-файли третіх сторін мають
залишатися локальним архівом поза public checkout. Звичайне видалення файлів у
новому commit не прибирає старі blobs з Git history; перед публічним релізом
потрібен окремий history rewrite або clean public export. Нижче залишено
систематизований індекс для наукового керівника: назви файлів, DOI, arXiv/source
URLs і роль у дипломі.

Якщо репозиторій передається приватно комісії разом із локальним source bundle,
PDF-архів можна зберігати поза Git. У Git слід тримати лише цей index,
source-capture notes і власні audit/traceability summaries.
