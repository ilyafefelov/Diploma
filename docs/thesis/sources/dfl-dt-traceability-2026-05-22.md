...
| Hugging Face Decision Transformer docs | DT is an offline trajectory model and should not be promoted without strong teacher trajectories. | DT/LAVA remains blocked until V10/V-next creates enough prior-supported, non-tail-risk material safe-switch labels; no raw hourly BUY/SELL/HOLD imitation is promoted. |
...
- V10 extends that ladder with a transfer audit: train/prior safe templates can
  be mined, but the learning-ceiling gate must decide whether those labels
  transfer safely enough for DT/LAVA or whether the current candidate space is
  exhausted.
