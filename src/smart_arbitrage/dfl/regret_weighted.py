...
HORIZON_REGRET_WEIGHTED_CALIBRATION_STRATEGY_KIND: Final[str] = (
    "horizon_regret_weighted_forecast_calibration_benchmark"
)
HORIZON_REGRET_WEIGHTED_CALIBRATED_MODEL_NAMES: Final[dict[str, str]] = {
    "tft_silver_v0": "tft_horizon_regret_weighted_calibrated_v0",
    "nbeatsx_silver_v0": "nbeatsx_horizon_regret_weighted_calibrated_v0",
}
...
def build_horizon_regret_weighted_forecast_calibration_frame(
    evaluation_frame: pl.DataFrame,
    *,
    forecast_model_names: tuple[str, ...] = tuple(
        HORIZON_REGRET_WEIGHTED_CALIBRATED_MODEL_NAMES
    ),
    min_prior_anchors: int = 14,
    rolling_calibration_window_anchors: int = 28,
) -> pl.DataFrame:
    """Build pre-anchor horizon-specific price-bias corrections."""
    ...
    rows: list[dict[str, Any]] = []
    for tenant_id in sorted(set(str(value) for value in evaluation_frame["tenant_id"].to_list())):
        tenant_frame = evaluation_frame.filter(pl.col("tenant_id") == tenant_id)
        for source_model_name in forecast_model_names:
            corrected_model_name = _horizon_corrected_model_name(source_model_name)
            model_frame = (
                tenant_frame
                .filter(pl.col("forecast_model_name") == source_model_name)
                .sort("anchor_timestamp")
...
def build_horizon_regret_weighted_forecast_strategy_benchmark_frame(
    evaluation_frame: pl.DataFrame,
    calibration_frame: pl.DataFrame,
) -> pl.DataFrame:
    """Evaluate horizon-aware corrected forecasts through the strict LP."""

    _validate_evaluation_frame(evaluation_frame)
    _validate_horizon_calibration_frame(calibration_frame)
    if evaluation_frame.height == 0 or calibration_frame.height == 0:
        return pl.DataFrame()
    ...
    for tenant_id, anchor_timestamp in tenant_anchors:
        ...
        for source_model_name in ["tft_silver_v0", "nbeatsx_silver_v0"]:
            ...
            candidates.append(
                ForecastCandidate(
                    model_name=corrected_model_name,
                    forecast_frame=_forecast_frame_from_payload_with_horizon_biases(
                        source_payload,
                        price_column_name="predicted_price_uah_mwh",
        ...
        evaluation = evaluate_forecast_candidates_against_oracle(
            ...
            anchor_timestamp=anchor_timestamp,
            candidates=candidates,
            evaluation_id=_horizon_calibration_evaluation_id(
                tenant_id=tenant_id,
                anchor_timestamp=anchor_timestamp,
            ),
            generated_at=generated_at,
        ...
        result_frames.append(
            _with_horizon_calibration_metadata(
                evaluation,
                source_payload=strict_payload,
                calibration_payloads=calibration_payloads,
                capacity_mwh=tenant_defaults.metrics.capacity_mwh,
...
def _validate_horizon_calibration_frame(calibration_frame: pl.DataFrame) -> None:
    required_columns = {
        "tenant_id",
        "anchor_timestamp",
        "source_forecast_model_name",
...
def _horizon_corrected_model_name(source_model_name: str) -> str:
    if source_model_name not in HORIZON_REGRET_WEIGHTED_CALIBRATED_MODEL_NAMES:
        raise ValueError(f"Unsupported forecast model for horizon calibration: {source_model_name}")
    return HORIZON_REGRET_WEIGHTED_CALIBRATED_MODEL_NAMES[source_model_name]
...
def _forecast_frame_from_payload_with_horizon_biases(
    payload: dict[str, Any],
    *,
    price_column_name: str,
    ...
    horizon_rows = _horizon_rows(payload)
    if len(horizon_biases) != len(horizon_rows):
        raise ValueError("horizon_biases_uah_mwh must match the forecast horizon length.")
    return pl.DataFrame(
        {
            "forecast_timestamp": [
                _datetime_value(row["interval_start"], field_name="interval_start")
            ...
            price_column_name: [
                float(row["forecast_price_uah_mwh"]) + horizon_biases[index]
...
def _horizon_calibration_evaluation_id(*, tenant_id: str, anchor_timestamp: datetime) -> str:
    return f"{tenant_id}:horizon-regret-weighted-calibration:{anchor_timestamp.strftime('%Y%m%dT%H%M')}"
...
def _with_horizon_calibration_metadata(
    evaluation: pl.DataFrame,
    *,
    source_payload: dict[str, Any],
    calibration_payloads: dict[str, dict[str, Any]],
    ...
    for row in evaluation.iter_rows(named=True):
        ...
        payload.update(
            {
                "benchmark_kind": HORIZON_REGRET_WEIGHTED_CALIBRATION_STRATEGY_KIND,
                "academic_scope": (
                    "Horizon-aware regret-weighted forecast calibration benchmark; "
                    "not full differentiable DFL."
                ),
                "data_quality_tier": str(source_payload.get("data_quality_tier", "demo_grade")),
                "observed_coverage_ratio": float(source_payload.get("observed_coverage_ratio", 0.0)),
                "efc_proxy": efc_proxy,
    ...
    return evaluation.with_columns(
        [
            pl.lit(HORIZON_REGRET_WEIGHTED_CALIBRATION_STRATEGY_KIND).alias(
                "strategy_kind"
            ),
            pl.Series("evaluation_payload", payloads),
            pl.Series("efc_proxy", efc_values),
...
def _weighted_horizon_biases(
    ...
    *,
    ...
    for prior_row in prior_rows:
        payload = _mapping(prior_row["evaluation_payload"])
