...
DFL_LAVA_SCHEDULE_NEIGHBOR_DT_CLAIM_SCOPE: Final[str] = (
    "dfl_lava_schedule_neighbor_dt_supervision_not_full_dfl"
)
DFL_LAVA_SCHEDULE_NEIGHBOR_DT_STRICT_CLAIM_SCOPE: Final[str] = (
    "dfl_lava_schedule_neighbor_dt_strict_lp_gate_not_full_dfl"
)
DFL_LAVA_SCHEDULE_NEIGHBOR_DT_STRICT_LP_STRATEGY_KIND: Final[str] = (
    "dfl_lava_schedule_neighbor_dt_strict_lp_benchmark"
)
DFL_LAVA_SCHEDULE_NEIGHBOR_DT_MODEL_NAME: Final[str] = (
    "dfl_lava_schedule_neighbor_dt_policy_v1"
)
DFL_LAVA_SCHEDULE_NEIGHBOR_DT_SELECTION_ROLE: Final[str] = (
    "lava_schedule_neighbor_dt_policy"
)
DFL_LAVA_SCHEDULE_NEIGHBOR_DT_BC_SELECTION_ROLE: Final[str] = (
    "schedule_neighbor_behavior_cloning_reference"
)
...
def build_dfl_lava_schedule_neighbor_dt_training_frame(
    lava_tail_risk_avoidance_label_frame: pl.DataFrame,
) -> pl.DataFrame:
    """Create DT/LAVA teacher rows over schedule-neighbor candidates.

    This is deliberately not hourly BUY/SELL/HOLD imitation. Each row keeps a
    feasible candidate schedule and assigns a schedule-neighbor class that a
    later DT can learn to choose or avoid.
    """

    _validate_tail_risk_avoidance_label_frame(lava_tail_risk_avoidance_label_frame)
    output_rows: list[dict[str, Any]] = []
    for row in lava_tail_risk_avoidance_label_frame.iter_rows(named=True):
        delta = float(row["label_regret_delta_vs_v2_plus_uah"])
        candidate_source = str(row["candidate_source"])
        eligible = bool(row["eligible_for_final_selection"])
        avoidance_class = str(row["tail_risk_avoidance_class"])
        if candidate_source == "v2_plus_default":
            teacher_class = "fallback_v2_plus"
        elif not eligible:
            teacher_class = "oracle_neighbor_diagnostic"
        elif avoidance_class == "safe_switch_win":
            teacher_class = "safe_schedule_neighbor"
        elif avoidance_class == "tail_risk_switch":
            teacher_class = "avoid_tail_risk_neighbor"
        else:
            teacher_class = "neutral_schedule_neighbor"
        copied = dict(row)
        copied.update(
            {
                "teacher_schedule_neighbor_class": teacher_class,
                "teacher_candidate_key": _candidate_key(row),
                ...
                "teacher_loss_weight": _dt_teacher_loss_weight(
                    teacher_class,
                    delta=delta,
                ),
                "is_training_row": (
                ...
                "raw_hourly_action_imitation": False,
                "claim_scope": DFL_LAVA_SCHEDULE_NEIGHBOR_DT_CLAIM_SCOPE,
                "not_full_dfl": True,
                "not_market_execution": True,
                "market_execution_enabled": False,
            }
...
def build_dfl_lava_schedule_neighbor_dt_policy_frame(
    lava_schedule_neighbor_dt_training_frame: pl.DataFrame,
    *,
    tenant_ids: tuple[str, ...],
    min_prior_safe_win_count: int = 2,
    max_prior_tail_loss_count: int = 0,
    min_prior_precision: float = 0.75,
    min_prior_mean_improvement_uah: float = 1.0,
    min_return_to_go_delta_uah: float = 1.0,
    ...
    """Fit a conservative schedule-neighbor policy from v3 teacher labels."""

    _validate_schedule_neighbor_dt_training_frame(
        lava_schedule_neighbor_dt_training_frame
    )
    if not tenant_ids:
        raise ValueError("tenant_ids must not be empty.")
    if min_prior_safe_win_count < 1:
    ...
    if min_prior_mean_improvement_uah < 0.0:
        raise ValueError("min_prior_mean_improvement_uah must not be negative.")
    if min_return_to_go_delta_uah < 0.0:
        raise ValueError("min_return_to_go_delta_uah must not be negative.")
    if not 0.0 <= max_dt_tail_risk_probability <= 1.0:
        raise ValueError("max_dt_tail_risk_probability must be between 0 and 1.")
    if ridge_l2 < 0.0:
        raise ValueError("ridge_l2 must not be negative.")
    if not allowed_candidate_sources:
        raise ValueError("allowed_candidate_sources must not be empty.")
    rows = list(lava_schedule_neighbor_dt_training_frame.iter_rows(named=True))
    allowed_sources = set(allowed_candidate_sources)
    hard_blocked = set(hard_blocked_candidate_families)
    output_rows: list[dict[str, Any]] = []
    for tenant_id in tenant_ids:
        ...
        if not train_rows:
            raise ValueError(f"{tenant_id} schedule-neighbor DT needs train rows.")
        if not final_rows:
            raise ValueError(f"{tenant_id} schedule-neighbor DT needs final rows.")
        allowed_train_rows = [
            row for row in train_rows if str(row["candidate_source"]) in allowed_sources
        ]
        challenger_train_rows = [
        ...
        if not allowed_train_rows:
            raise ValueError(
                f"{tenant_id} schedule-neighbor DT needs allowed train rows."
            )
        if not challenger_train_rows:
            raise ValueError(
                f"{tenant_id} schedule-neighbor DT needs challenger train rows."
            )
        regret_scorer = _fit_safe_switch_v2_scorer(
            challenger_train_rows,
            ridge_l2=ridge_l2,
        ...
        tail_risk_train_rows = [
            {
                **row,
                "label_tail_risk_probability_target": (
                    1.0
                    if str(row["teacher_schedule_neighbor_class"])
        ...
        profile_stats = _dt_schedule_neighbor_profile_stats(
            train_rows,
            allowed_candidate_sources=allowed_sources,
            hard_blocked_candidate_families=hard_blocked,
        )
        allowed_profiles = sorted(
            ...
            and float(stats["safe_precision"]) >= min_prior_precision
            and float(stats["mean_prior_delta_uah"]) <= -min_prior_mean_improvement_uah
            and float(stats["mean_return_to_go_delta_uah"]) >= min_return_to_go_delta_uah
            and not bool(stats["hard_blocked"])
        )
        selected_final: list[dict[str, Any]] = []
        fallback_anchor_keys: list[str] = []
        ...
        for anchor, anchor_rows in sorted(_rows_by_anchor(final_rows).items()):
            candidates = [
                ...
                if str(row["candidate_source"]) in allowed_sources
                and str(row["candidate_family"]) not in hard_blocked
                and _dt_schedule_neighbor_profile_key(row) in allowed_profiles
            ]
            scored: list[tuple[dict[str, Any], float, float]] = []
            ...
            switchable = [
                (row, predicted_delta, predicted_tail_risk)
                for row, predicted_delta, predicted_tail_risk in scored
                if predicted_delta <= -min_return_to_go_delta_uah
                and predicted_tail_risk <= max_dt_tail_risk_probability
            ]
            if not switchable:
                fallback_anchor_keys.append(_anchor_key_from_parts(tenant_id, anchor[1]))
                continue
            selected_final.append(
                min(
                    ...
                    key=lambda item: (
                        ...
                        item[1],
                        float(
                            profile_stats[_dt_schedule_neighbor_profile_key(item[0])][
                                "mean_prior_delta_uah"
                            ]
                        ),
                        float(item[0]["selector_feature_schedule_distance_from_v2_plus"]),
                        abs(
                            float(item[0]["selector_feature_total_throughput_delta_mwh"])
                        ),
                        str(item[0]["candidate_family"]),
                        str(item[0]["candidate_model_name"]),
                    ),
        ...
        if fallback_count:
            selected_family_counts["frozen_v2_plus_fallback"] = fallback_count
        if selected_final:
            blocker = "schedule_neighbor_teacher_profile_selected"
        elif not allowed_profiles:
            blocker = "no_prior_safe_schedule_neighbor_profile"
        else:
            blocker = "no_candidate_after_schedule_neighbor_tail_risk_filter"
        output_rows.append(
            {
                "tenant_id": tenant_id,
                "learner_model_name": DFL_LAVA_SCHEDULE_NEIGHBOR_DT_MODEL_NAME,
                "target_label_space": "schedule_neighbor_candidate_index",
                "raw_hourly_action_imitation": False,
                "selected_scorer_type": "schedule_neighbor_dt_teacher_profile_v1",
                "selected_feature_names": list(_SAFE_SWITCH_V2_FEATURE_COLUMNS),
                "allowed_schedule_neighbor_profiles": allowed_profiles,
                "blocked_schedule_neighbor_profiles": sorted(
                    profile
                    for profile, stats in profile_stats.items()
                    if profile not in allowed_profiles or bool(stats["hard_blocked"])
                ),
                "schedule_neighbor_profile_prior_stats": profile_stats,
                "fallback_to_v2_plus": not selected_final,
                "uses_v2_plus_anchor_fallback": bool(fallback_anchor_keys),
                "selector_gate_blocker": blocker,
                "hard_blocked_candidate_families": sorted(hard_blocked),
                "allowed_candidate_sources": sorted(allowed_sources),
                "min_return_to_go_delta_uah": min_return_to_go_delta_uah,
                "max_dt_tail_risk_probability": max_dt_tail_risk_probability,
                "selected_feature_weights": dict(regret_scorer["weights"]),
                "selected_tail_risk_feature_weights": dict(
                    tail_risk_scorer["weights"]
                ),
                "selected_feature_means": dict(regret_scorer["feature_means"]),
                "selected_feature_scales": dict(regret_scorer["feature_scales"]),
                "train_anchor_count": _anchor_count(train_rows),
                ...
                "selected_final_profile_keys": [
                    _dt_schedule_neighbor_profile_key(row) for row in selected_final
                ],
                "selected_final_family_counts": selected_family_counts,
                "selected_final_candidate_source_counts": selected_counts,
                "predicted_final_candidate_deltas": {
    ...
    return pl.DataFrame(output_rows, infer_schema_length=None).sort(["tenant_id"])


def build_dfl_lava_schedule_neighbor_dt_strict_lp_benchmark_frame(
    lava_schedule_neighbor_dt_training_frame: pl.DataFrame,
    lava_schedule_neighbor_dt_policy_frame: pl.DataFrame,
    frozen_v2_plus_strict_frame: pl.DataFrame,
    *,
    baseline_source_model_name: str = FROZEN_V2_PLUS_BASELINE_MODEL_NAME,
    generated_at: datetime | None = None,
) -> pl.DataFrame:
    """Strict-score the DT/LAVA schedule-neighbor policy against V2+."""

    _validate_schedule_neighbor_dt_training_frame(
        lava_schedule_neighbor_dt_training_frame
    )
    _require_columns(
        lava_schedule_neighbor_dt_policy_frame,
        frozenset(
            {
                "tenant_id",
                "selected_final_candidate_keys",
        ...
        frame_name="lava_schedule_neighbor_dt_policy_frame",
    )
    _require_columns(
        frozen_v2_plus_strict_frame,
        _REQUIRED_STRICT_COLUMNS,
    ...
    candidate_rows = list(lava_schedule_neighbor_dt_training_frame.iter_rows(named=True))
    candidate_by_key = {_candidate_key(row): row for row in candidate_rows}
    v2_reference_by_anchor: dict[str, dict[str, Any]] = {}
    output_rows: list[dict[str, Any]] = []
    for row in frozen_v2_plus_strict_frame.iter_rows(named=True):
        ...
        if (
            ...
            v2_reference_by_anchor[_anchor_key(row)] = row
        output_rows.append(
            _schedule_neighbor_dt_reference_row(
                row,
                selection_role=role,
                generated_at=resolved_generated_at,
            )
        )
    for policy_row in lava_schedule_neighbor_dt_policy_frame.iter_rows(named=True):
        tenant_id = str(policy_row["tenant_id"])
        for key in policy_row["selected_final_candidate_keys"]:
            selected = candidate_by_key[str(key)]
            output_rows.append(
                _schedule_neighbor_dt_candidate_row(
                    selected,
                    policy_row=policy_row,
                    selection_role=DFL_LAVA_SCHEDULE_NEIGHBOR_DT_SELECTION_ROLE,
                    generated_at=resolved_generated_at,
                )
            )
        for anchor_key in policy_row["fallback_final_anchor_keys"]:
            ...
            if fallback is None:
                raise ValueError(f"Missing frozen V2+ fallback row for {anchor_key}.")
            output_rows.append(
                _schedule_neighbor_dt_fallback_row(
                    fallback,
                    policy_row=policy_row,
                    generated_at=resolved_generated_at,
                )
            )
        for selected in _schedule_neighbor_dt_behavior_cloning_rows(
            candidate_rows,
            tenant_id=tenant_id,
        ):
            output_rows.append(
                _schedule_neighbor_dt_candidate_row(
                    selected,
                    policy_row=policy_row,
                    selection_role=DFL_LAVA_SCHEDULE_NEIGHBOR_DT_BC_SELECTION_ROLE,
                    generated_at=resolved_generated_at,
                )
            )
    return pl.DataFrame(output_rows, infer_schema_length=None).sort(
...
def evaluate_dfl_lava_schedule_neighbor_dt_gate(
    strict_frame: pl.DataFrame,
    *,
    min_validation_tenant_anchor_count: int = 90,
    min_mean_regret_improvement_ratio_vs_v2_plus: float = 0.0,
    ...
    """Gate the schedule-neighbor DT/LAVA policy against frozen V2+."""
    ...
    _require_columns(
        ...
        frame_name="schedule-neighbor DT strict frame",
    )
    return _evaluate_against_v2_plus(
        strict_frame,
        selected_role=DFL_LAVA_SCHEDULE_NEIGHBOR_DT_SELECTION_ROLE,
        min_validation_tenant_anchor_count=min_validation_tenant_anchor_count,
        min_mean_regret_improvement_ratio_vs_v2_plus=(
            min_mean_regret_improvement_ratio_vs_v2_plus
        ),
        ...
        success_description=(
            "Schedule-neighbor DT/LAVA policy beats V2+ under strict LP/oracle evidence"
        ),
    )
...
def _validate_schedule_neighbor_dt_training_frame(frame: pl.DataFrame) -> None:
    _validate_tail_risk_avoidance_label_frame(frame)
    _require_columns(
        frame,
        frozenset(
            {
        ...
        frame_name="schedule-neighbor DT training frame",
    )
    if set(frame["target_label_space"].unique().to_list()) != {
        "schedule_neighbor_candidate_index"
    }:
        raise ValueError("schedule-neighbor DT target must be candidate-index based.")
    if frame.select(pl.col("raw_hourly_action_imitation").any()).item():
        raise ValueError("schedule-neighbor DT training refuses hourly action imitation.")


def _dt_teacher_loss_weight(teacher_class: str, *, delta: float) -> float:
    if teacher_class == "safe_schedule_neighbor":
        return max(1.0, min(10.0, abs(delta) / 25.0))
    if teacher_class == "avoid_tail_risk_neighbor":
        return max(1.0, min(10.0, delta / 50.0))
    if teacher_class == "fallback_v2_plus":
        return 1.0
    return 0.25


def _dt_schedule_neighbor_profile_stats(
    ...
    *,
    ...
    for row in train_rows:
        if str(row["candidate_source"]) not in allowed_candidate_sources:
            continue
        profile = _dt_schedule_neighbor_profile_key(row)
        family = str(row["candidate_family"])
        teacher_class = str(row["teacher_schedule_neighbor_class"])
        ...
        elif teacher_class == "avoid_tail_risk_neighbor":
            item["tail_loss_count"] += 1
        else:
            item["weak_count"] += 1
        item["deltas"].append(float(row["label_regret_delta_vs_v2_plus_uah"]))
        item["return_to_go_deltas"].append(
            float(row["teacher_return_to_go_delta_uah"])
        )
    ...
    for profile, values in raw_stats.items():
        ...
        stats[profile] = {
            ...
            "mean_return_to_go_delta_uah": (
                mean(values["return_to_go_deltas"])
                if values["return_to_go_deltas"]
                else 0.0
            ),
...
def _dt_schedule_neighbor_profile_key(row: dict[str, Any]) -> str:
    return "|".join(
        (
            ...
            "pl_ua_spread_ratio="
            + _signed_bucket(
                float(row.get("selector_feature_poland_lag24_ua_spread_ratio", 0.0))
            ),
...
def _schedule_neighbor_dt_reference_row(
    row: dict[str, Any],
    *,
    selection_role: str,
    generated_at: datetime,
    ...
    copied.update(
        {
            "evaluation_id": (
                f"{row['tenant_id']}:lava_schedule_neighbor_dt:{selection_role}:"
                f"{_datetime_value(row['anchor_timestamp']):%Y%m%dT%H%M}"
            ),
            ...
            "selected_strategy_source": selection_role,
            "generated_at": generated_at,
            "claim_scope": DFL_LAVA_SCHEDULE_NEIGHBOR_DT_STRICT_CLAIM_SCOPE,
            "target_label_space": "schedule_neighbor_candidate_index",
            "raw_hourly_action_imitation": False,
            "not_full_dfl": True,
            "not_market_execution": True,
...
def _schedule_neighbor_dt_fallback_row(
    row: dict[str, Any],
    *,
    ...
    payload.update(
        {
            "selector_role": DFL_LAVA_SCHEDULE_NEIGHBOR_DT_SELECTION_ROLE,
            "fallback_to_v2_plus": True,
            "target_label_space": "schedule_neighbor_candidate_index",
            "raw_hourly_action_imitation": False,
            "allowed_schedule_neighbor_profiles": list(
                policy_row["allowed_schedule_neighbor_profiles"]
            ),
            "max_dt_tail_risk_probability": float(
                policy_row["max_dt_tail_risk_probability"]
            ),
            "market_execution_enabled": False,
        }
    )
    copied.update(
        {
            "evaluation_id": (
                f"{row['tenant_id']}:lava_schedule_neighbor_dt:"
                f"{DFL_LAVA_SCHEDULE_NEIGHBOR_DT_SELECTION_ROLE}:"
                f"{_datetime_value(row['anchor_timestamp']):%Y%m%dT%H%M}"
            ),
            "source_model_name": "lava_schedule_neighbor_dt_bridge_v1",
            "forecast_model_name": DFL_LAVA_SCHEDULE_NEIGHBOR_DT_MODEL_NAME,
            "strategy_kind": DFL_LAVA_SCHEDULE_NEIGHBOR_DT_STRICT_LP_STRATEGY_KIND,
            "selection_role": DFL_LAVA_SCHEDULE_NEIGHBOR_DT_SELECTION_ROLE,
            "selected_strategy_source": "frozen_v2_plus_fallback",
            "selected_candidate_family": "frozen_v2_plus_fallback",
...
def _schedule_neighbor_dt_candidate_row(
    row: dict[str, Any],
    *,
    policy_row: dict[str, Any],
    selection_role: str,
    ...
    selected_key = _candidate_key(row)
    predicted_deltas = dict(policy_row.get("predicted_final_candidate_deltas", {}))
    predicted_tail_risks = dict(
        policy_row.get("predicted_final_tail_risk_probabilities", {})
    )
    payload = _payload(row)
    payload.update(
        {
            ...
            "target_label_space": "schedule_neighbor_candidate_index",
            "raw_hourly_action_imitation": False,
            "selected_schedule_neighbor_profile": (
                _dt_schedule_neighbor_profile_key(row)
            ),
            "allowed_schedule_neighbor_profiles": list(
                policy_row["allowed_schedule_neighbor_profiles"]
            ),
            "teacher_schedule_neighbor_class": str(
                row["teacher_schedule_neighbor_class"]
            ),
            "teacher_return_to_go_delta_uah": float(
                row["teacher_return_to_go_delta_uah"]
            ),
            "predicted_regret_delta_vs_v2_plus_uah": predicted_deltas.get(
                selected_key,
                None,
            ),
            "predicted_tail_risk_probability": predicted_tail_risks.get(
                selected_key,
                None,
            ),
            "max_dt_tail_risk_probability": float(
    ...
    model_name = (
        DFL_LAVA_SCHEDULE_NEIGHBOR_DT_MODEL_NAME
        if selection_role == DFL_LAVA_SCHEDULE_NEIGHBOR_DT_SELECTION_ROLE
        else f"{DFL_LAVA_SCHEDULE_NEIGHBOR_DT_MODEL_NAME}_behavior_cloning"
    )
    return {
        "evaluation_id": (
            f"{row['tenant_id']}:lava_schedule_neighbor_dt:{selection_role}:"
        ...
        "generated_at": generated_at,
        "horizon_hours": int(row["horizon_hours"]),
        "starting_soc_fraction": _first_or_default(
            row["soc_fraction_vector"],
            default=0.5,
        ),
        "starting_soc_source": "lava_schedule_neighbor_dt_bridge",
        "regret_uah": float(row["regret_uah"]),
        "regret_ratio": float(row["regret_ratio"]),
        "decision_value_uah": float(row["decision_value_uah"]),
        "forecast_objective_value_uah": float(row["forecast_objective_value_uah"]),
        ...
        "committed_power_mw": abs(
            _first_or_default(row["dispatch_mw_vector"], default=0.0)
        ),
        "rank_by_regret": 1,
        "data_quality_tier": str(row.get("data_quality_tier", "thesis_grade")),
        "observed_coverage_ratio": float(row.get("observed_coverage_ratio", 1.0)),
...
def _schedule_neighbor_dt_behavior_cloning_rows(
    rows: list[dict[str, Any]],
    *,
    tenant_id: str,
) -> list[dict[str, Any]]:
    ...
    train_winners = [
        ...
        and str(row["teacher_schedule_neighbor_class"]) == "safe_schedule_neighbor"
    ]
    if train_winners:
    ...
    for _anchor, anchor_rows in sorted(_rows_by_anchor(final_rows).items()):
        matching = [
            row for row in anchor_rows if str(row["candidate_family"]) == selected_family
        ]
        fallback = [
            row for row in anchor_rows if str(row["candidate_source"]) == "v2_plus_default"
        ]
