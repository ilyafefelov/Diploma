...
class DflRegretSurrogateV1AssetConfig(DflUaContextSafeSwitchAssetConfig):
    ...
    allowed_candidate_sources_csv: str = (
        "ua_context_v8_generated_candidate,v7_generated_candidate,oracle_gap_candidate,poland_shadow_candidate,"
        "tft_shadow_candidate,ua_context_candidate,lava_candidate"
    )
...
def entsoe_poland_lagged_feature_candidate_frame(
    ...
    _add_metadata(
        context,
        {
            ...
            "coverage_status": lagged_frame.select("coverage_status")
            .to_series()
            .item(0)
...
def dfl_official_global_panel_schedule_value_learner_v2_plus_oracle_gap_audit_frame(
    ...
    _add_metadata(
        ...
        {
            "rows": audit_frame.height,
            "oracle_gap_classes": sorted(
                audit_frame["oracle_gap_class"].unique().to_list()
            )
            if audit_frame.height
            else [],
...
def dfl_ua_context_safe_switch_scorer_frame(
    ...
    frame = build_dfl_ua_context_safe_switch_scorer_frame(
        ...
        tenant_ids=_csv_values(config.tenant_ids_csv, field_name="tenant_ids_csv"),
        forecast_model_names=_forecast_model_names(config.source_model_names_csv),
        scorer_kinds=_csv_values(
            config.scorer_kinds_csv, field_name="scorer_kinds_csv"
        ),
        min_prior_safe_win_count=config.min_prior_safe_win_count,
        min_prior_mean_improvement_uah=config.min_prior_mean_improvement_uah,
        min_predicted_improvement_uah=config.min_predicted_improvement_uah,
    ...
    _add_metadata(
        context,
        {
            ...
            "candidate_universe_can_beat_v2_plus_gate": (
                frame.select(
                    pl.col("candidate_universe_can_beat_v2_plus_gate").any()
                ).item()
                if frame.height
                else False
            ),
...
@dg.asset(
    ...
    tags=taxonomy.asset_tags(
        ...
        ml_stage="diagnostics",
        evidence_scope="not_market_execution",
        backend="candidate_value_v7",
        market_venue="DAM",
    ),
)
def dfl_v2_plus_opportunity_backfill_requirements_frame(
    context,
    config: DflRegretSurrogateV1AssetConfig,
    dfl_sparse_safe_switch_candidate_library_v6_frame: pl.DataFrame,
    dfl_sparse_safe_switch_opportunity_audit_frame: pl.DataFrame,
) -> pl.DataFrame:
    """Classify whether V2+ misses need backfill, new candidates, or DT readiness."""

    frame = build_dfl_v2_plus_opportunity_backfill_requirements_frame(
        dfl_sparse_safe_switch_candidate_library_v6_frame,
        dfl_sparse_safe_switch_opportunity_audit_frame,
        material_switch_delta_uah=config.material_switch_delta_uah,
    ...
    _add_metadata(
        ...
        {
            "rows": frame.height,
            "backfill_decisions": sorted(
                frame["opportunity_backfill_decision"].unique().to_list()
            )
            if frame.height
            else [],
            "candidate_generation_needed_rows": frame.filter(
                pl.col("opportunity_backfill_decision") == "candidate_generation_needed"
            ).height
            if frame.height
            else 0,
            "dt_ready_rows": frame.filter(
                pl.col("opportunity_backfill_decision") == "dt_ready"
            ).height
            if frame.height
            else 0,
            "market_execution_enabled": False,
            "scope": "dfl_v2_plus_opportunity_backfill_requirements_not_full_dfl",
            "not_market_execution": True,
        },
    )
    return frame
...
def dfl_backfilled_context_feature_panel_v7_frame(
    context,
    dfl_sparse_safe_switch_candidate_library_v6_frame: pl.DataFrame,
    dfl_v2_plus_opportunity_backfill_requirements_frame: pl.DataFrame,
) -> pl.DataFrame:
    """Attach prior-only context backfill features for V7."""

    frame = build_dfl_backfilled_context_feature_panel_v7_frame(
        dfl_sparse_safe_switch_candidate_library_v6_frame,
    ...
    _add_metadata(
        context,
        {
            ...
            "market_execution_enabled": False,
            "scope": "dfl_backfilled_context_feature_panel_v7_not_full_dfl",
            "not_market_execution": True,
        },
    )
    return frame
...
def dfl_feasible_schedule_candidate_library_v7_frame(
    context,
    dfl_backfilled_context_feature_panel_v7_frame: pl.DataFrame,
    dfl_v2_plus_opportunity_backfill_requirements_frame: pl.DataFrame,
) -> pl.DataFrame:
    """Add V7 feasible schedule variants around V2+ misses."""

    frame = build_dfl_feasible_schedule_candidate_library_v7_frame(
    ...
    _add_metadata(
        context,
        {
            ...
            "v7_generated_candidate_rows": frame.filter(
...
def dfl_ua_context_candidate_v8_strict_rescore_frame(
    context,
    dfl_ua_context_feasible_schedule_candidate_library_v8_frame: pl.DataFrame,
) -> pl.DataFrame:
    """Strict-score V8 explicit schedules against realized prices and oracle value."""

    frame = build_dfl_ua_context_candidate_v8_strict_rescore_frame(
        dfl_ua_context_feasible_schedule_candidate_library_v8_frame
    )
    generated_frame = frame.filter(
        pl.col("candidate_source") == "ua_context_v8_generated_candidate"
    )
    generated_mean_regret = (
        generated_frame.select(pl.col("regret_uah").mean()).item()
        if generated_frame.height
        else None
    )
    _add_metadata(
        context,
        {
            "rows": frame.height,
            "strict_rescored_v8_candidate_rows": frame.filter(
                pl.col("candidate_value_label_status") == "strict_rescored_v8_candidate"
            ).height
            if frame.height
            else 0,
            "mean_generated_v8_regret_uah": generated_mean_regret,
            "target_label_space": "schedule_candidate_value_v8",
            "market_execution_enabled": False,
            "scope": "dfl_ua_context_candidate_v8_strict_rescore_not_full_dfl",
            "not_market_execution": True,
        },
    )
    return frame
...
def dfl_ua_context_candidate_value_teacher_label_panel_v8_frame(
    context,
    config: DflRegretSurrogateV1AssetConfig,
    dfl_ua_context_candidate_v8_strict_rescore_frame: pl.DataFrame,
    dfl_v2_plus_opportunity_backfill_requirements_frame: pl.DataFrame,
) -> pl.DataFrame:
    """Rebuild V8 candidate-value teacher labels after strict rescore."""

    frame = build_dfl_ua_context_candidate_value_teacher_label_panel_v8_frame(
    ...
    _add_metadata(
        context,
        {
            "rows": frame.height,
            "material_safe_switch_rows": frame.filter(
                pl.col("label_v8_material_safe_switch")
            ).height
            if frame.height
            else 0,
            "final_generated_material_safe_switch_rows": frame.filter(
                (pl.col("split_name") == "final_holdout")
                & (pl.col("candidate_source") == "ua_context_v8_generated_candidate")
                & pl.col("label_v8_material_safe_switch")
            ).height
            ...
            "raw_hourly_action_imitation": False,
            "market_execution_enabled": False,
            "scope": "dfl_ua_context_candidate_value_teacher_v8_not_full_dfl",
            "not_market_execution": True,
        },
    )
    return frame
...
def dfl_candidate_value_regret_surrogate_v8_frame(
    context,
    config: DflRegretSurrogateV1AssetConfig,
    dfl_ua_context_candidate_value_teacher_label_panel_v8_frame: pl.DataFrame,
) -> pl.DataFrame:
    """Select strict-rescored V8 schedules only with prior-safe evidence."""

    frame = build_dfl_candidate_value_regret_surrogate_v8_frame(
    ...
    _add_metadata(
        context,
        {
            ...
            "raw_hourly_action_imitation": False,
            "market_execution_enabled": False,
            "scope": "dfl_candidate_value_regret_surrogate_v8_not_full_dfl",
            "not_market_execution": True,
        },
    )
    return frame
...
def dfl_candidate_value_v8_strict_lp_benchmark_frame(
    context,
    config: DflRegretSurrogateV1AssetConfig,
    dfl_ua_context_candidate_value_teacher_label_panel_v8_frame: pl.DataFrame,
    ...
    strict_frame = build_dfl_candidate_value_v8_strict_lp_benchmark_frame(
        dfl_ua_context_candidate_value_teacher_label_panel_v8_frame,
    ...
    _add_metadata(
        ...
        {
            "rows": strict_frame.height,
            "strategy_kind": REGRET_SURROGATE_CANDIDATE_VALUE_V8_STRICT_LP_STRATEGY_KIND,
            "gate_decision": gate.decision,
            "gate_description": gate.description,
            "diagnostic_signal_passed": gate.metrics.get(
                "diagnostic_signal_passed", False
            ...
            "raw_hourly_action_imitation": False,
            "market_execution_enabled": False,
            "scope": "dfl_candidate_value_v8_strict_lp_gate_not_full_dfl",
            "not_market_execution": True,
        },
    )
    return strict_frame
...
def dfl_candidate_value_v8_rolling_robustness_frame(
    context,
    config: DflRegretSurrogateV1AssetConfig,
    dfl_ua_context_candidate_v8_strict_rescore_frame: pl.DataFrame,
    dfl_v2_plus_opportunity_backfill_requirements_frame: pl.DataFrame,
) -> pl.DataFrame:
    """Rolling prior-only robustness for V8 candidate-value selection."""

    frame = build_dfl_candidate_value_v8_rolling_robustness_frame(
        dfl_ua_context_candidate_v8_strict_rescore_frame,
        dfl_v2_plus_opportunity_backfill_requirements_frame,
        tenant_ids=_csv_values(config.tenant_ids_csv, field_name="tenant_ids_csv"),
        source_model_names=_forecast_model_names(config.source_model_names_csv),
    ...
    _add_metadata(
        context,
        {
            ...
            "backfill_required_family_rows": frame.filter(
                pl.col("recommended_next_action") == "backfill_ukrainian_prior_context"
            ).height
            if frame.height
            else 0,
            "target_label_space": "schedule_candidate_value_v8",
...
@dg.asset(
    ...
    tags=taxonomy.asset_tags(
        ...
        ml_stage="training_data",
        evidence_scope="not_market_execution",
        backend="candidate_value_v9",
        market_venue="DAM",
    ),
)
def dfl_ua_prior_context_backfilled_feature_panel_v9_frame(
    context,
    config: DflRegretSurrogateV1AssetConfig,
    dfl_v8_pruned_candidate_value_teacher_label_panel_frame: pl.DataFrame,
) -> pl.DataFrame:
    """Attach stronger Ukrainian prior-support features before new candidates."""

    frame = build_dfl_ua_prior_context_backfilled_feature_panel_v9_frame(
        dfl_v8_pruned_candidate_value_teacher_label_panel_frame,
        min_prior_context_neighbor_count=config.min_prior_context_neighbor_count,
    )
    _add_metadata(
        context,
        {
            "rows": frame.height,
            "context_ready_rows": frame.filter(
                pl.col("selector_feature_v9_prior_context_ready") >= 1.0
            ).height
            if frame.height
            else 0,
            "context_blockers": sorted(
            ...
            "raw_hourly_action_imitation": False,
            "market_execution_enabled": False,
            "scope": "dfl_ua_prior_context_backfilled_feature_panel_v9_not_full_dfl",
            "not_market_execution": True,
        },
    )
    return frame
...
def dfl_ua_non_tail_risk_candidate_library_v9_frame(
    context,
    dfl_ua_prior_context_backfilled_feature_panel_v9_frame: pl.DataFrame,
) -> pl.DataFrame:
    """Generate bounded same-energy candidate schedules after V8 pruning."""

    frame = build_dfl_ua_non_tail_risk_candidate_library_v9_frame(
    ...
    _add_metadata(
        ...
        {
            "rows": frame.height,
            "v9_generated_candidate_rows": frame.filter(
            ...
            "raw_hourly_action_imitation": False,
            "market_execution_enabled": False,
            "scope": "dfl_ua_non_tail_risk_candidate_library_v9_not_full_dfl",
            "not_market_execution": True,
        },
    )
    return frame
...
def dfl_ua_non_tail_risk_candidate_v9_strict_rescore_frame(
    context,
    dfl_ua_non_tail_risk_candidate_library_v9_frame: pl.DataFrame,
) -> pl.DataFrame:
    """Strict-score V9 non-tail-risk candidates with unchanged LP/oracle labels."""

    frame = build_dfl_ua_non_tail_risk_candidate_v9_strict_rescore_frame(
    ...
    _add_metadata(
        ...
        {
            "rows": frame.height,
            "strict_rescored_v9_candidate_rows": frame.filter(
                pl.col("candidate_value_label_status") == "strict_rescored_v9_candidate"
            ).height
            if frame.height
            else 0,
            "mean_generated_v9_regret_uah": generated_mean_regret,
            "target_label_space": "schedule_candidate_value_v9",
            "raw_hourly_action_imitation": False,
            "market_execution_enabled": False,
            "scope": "dfl_ua_non_tail_risk_candidate_v9_strict_rescore_not_full_dfl",
            "not_market_execution": True,
        },
    )
    return frame
...
def dfl_ua_non_tail_risk_candidate_value_teacher_label_panel_v9_frame(
    context,
    config: DflRegretSurrogateV1AssetConfig,
    dfl_ua_non_tail_risk_candidate_v9_strict_rescore_frame: pl.DataFrame,
) -> pl.DataFrame:
    """Rebuild teacher labels from strict-scored non-tail-risk V9 candidates."""

    frame = build_dfl_ua_non_tail_risk_candidate_value_teacher_label_panel_v9_frame(
    ...
    _add_metadata(
        context,
        {
            "rows": frame.height,
            "material_safe_switch_rows": frame.filter(
                pl.col("label_v9_material_safe_switch")
            ).height
            if frame.height
            else 0,
            "final_generated_material_safe_switch_rows": frame.filter(
            ...
            "tail_risk_generated_rows": frame.filter(
                (pl.col("candidate_source") == "ua_context_v9_generated_candidate")
                & pl.col("label_v9_tail_risk_loss")
            ).height
            if frame.height
            else 0,
            "selector_training_eligible_generated_rows": frame.filter(
                (pl.col("candidate_source") == "ua_context_v9_generated_candidate")
                & pl.col("eligible_for_next_selector_training_v9")
            ).height
            if frame.height
            else 0,
            "target_label_space": "schedule_candidate_value_v9",
...
def dfl_ua_calendar_block_context_backfill_frame(
    ...
    _add_metadata(
        context,
        {
            ...
            "blocker_values": sorted(
                str(value) for value in frame["calendar_block_backfill_status"].unique()
            )
            if frame.height
            else [],
            "market_execution_enabled": False,
    ...
    _add_metadata(
        context,
        {
            ...
            "gate_decisions": sorted(
                str(value) for value in frame["context_backfill_gate_decision"].unique()
            )
            if frame.height
            else [],
            "market_execution_enabled": False,
...
@dg.asset(
    ...
    tags=taxonomy.asset_tags(
        ...
        ml_stage="training_data",
        evidence_scope="not_market_execution",
        backend="candidate_value_v11",
        market_venue="DAM",
    ),
)
def dfl_lower_tail_risk_candidate_library_v11_frame(
    context,
    config: DflRegretSurrogateV1AssetConfig,
    dfl_oracle_template_candidate_value_teacher_label_panel_v10_frame: pl.DataFrame,
    dfl_ua_context_backfill_coverage_gate_frame: pl.DataFrame,
) -> pl.DataFrame:
    """Generate source-gated lower-tail-risk V11 candidate schedules."""

    frame = build_dfl_lower_tail_risk_candidate_library_v11_frame(
        dfl_oracle_template_candidate_value_teacher_label_panel_v10_frame,
        dfl_ua_context_backfill_coverage_gate_frame,
        min_prior_non_tail_risk_support_count=0,
        max_prior_tail_risk_probability=config.max_neighbor_tail_risk_probability,
    )
    generated = frame.filter(
    ...
    _add_metadata(
        context,
        {
            ...
            "ready_rows": frame.filter(pl.col("v11_candidate_generation_ready")).height
            if frame.height
            else 0,
            "candidate_schedule_classes": sorted(
            ...
            "raw_hourly_action_imitation": False,
            "market_execution_enabled": False,
            "scope": "dfl_lower_tail_risk_candidate_library_v11_not_full_dfl",
            "not_market_execution": True,
        },
    )
    return frame
...
def dfl_lower_tail_risk_candidate_v11_strict_rescore_frame(
    context,
    dfl_lower_tail_risk_candidate_library_v11_frame: pl.DataFrame,
) -> pl.DataFrame:
    """Strict-score V11 candidates under the unchanged LP/oracle labels."""

    frame = build_dfl_lower_tail_risk_candidate_v11_strict_rescore_frame(
    ...
    _add_metadata(
        ...
        {
            "rows": frame.height,
            "strict_rescored_v11_rows": generated.filter(
                pl.col("candidate_value_label_status")
            ...
            "raw_hourly_action_imitation": False,
            "market_execution_enabled": False,
            "scope": "dfl_lower_tail_risk_candidate_v11_strict_rescore_not_full_dfl",
            "not_market_execution": True,
        },
    )
    return frame
...
def dfl_lower_tail_risk_candidate_value_teacher_label_panel_v11_frame(
    context,
    config: DflRegretSurrogateV1AssetConfig,
    dfl_lower_tail_risk_candidate_v11_strict_rescore_frame: pl.DataFrame,
) -> pl.DataFrame:
    """Attach V11 candidate-value labels after strict rescore."""

    frame = build_dfl_lower_tail_risk_candidate_value_teacher_label_panel_v11_frame(
    ...
    _add_metadata(
        context,
        {
            ...
            "material_safe_switch_rows": generated.filter(
                pl.col("label_v11_material_safe_switch")
            ).height
            if generated.height
            else 0,
            "tail_risk_rows": generated.filter(
                pl.col("label_v11_tail_risk_loss")
            ).height
            if generated.height
            else 0,
            "target_label_space": "schedule_candidate_value_v11",
            "raw_hourly_action_imitation": False,
            "market_execution_enabled": False,
            "scope": "dfl_lower_tail_risk_candidate_value_teacher_v11_not_full_dfl",
            "not_market_execution": True,
        },
    )
    return frame
...
def dfl_candidate_value_regret_surrogate_v11_frame(
    context,
    config: DflRegretSurrogateV1AssetConfig,
    dfl_lower_tail_risk_candidate_value_teacher_label_panel_v11_frame: pl.DataFrame,
) -> pl.DataFrame:
    """Fit conservative V11 candidate-value safe-switch policy."""

    frame = build_dfl_candidate_value_regret_surrogate_v11_frame(
    ...
    _add_metadata(
        context,
        {
            ...
            "raw_hourly_action_imitation": False,
            "market_execution_enabled": False,
            "scope": "dfl_candidate_value_regret_surrogate_v11_not_full_dfl",
            "not_market_execution": True,
        },
    )
    return frame
...
def dfl_candidate_value_v11_strict_lp_benchmark_frame(
    context,
    dfl_lower_tail_risk_candidate_value_teacher_label_panel_v11_frame: pl.DataFrame,
    dfl_candidate_value_regret_surrogate_v11_frame: pl.DataFrame,
) -> pl.DataFrame:
    """Strict-score V11 selector versus strict and frozen V2+ references."""

    frame = build_dfl_candidate_value_v11_strict_lp_benchmark_frame(
        dfl_lower_tail_risk_candidate_value_teacher_label_panel_v11_frame,
        dfl_candidate_value_regret_surrogate_v11_frame,
    )
    v11_rows = frame.filter(
        pl.col("selection_role") == REGRET_SURROGATE_CANDIDATE_VALUE_V11_SELECTION_ROLE
    )
    v11_mean_regret: Any = v11_rows["regret_uah"].mean() if v11_rows.height else None
    _add_metadata(
        context,
        {
            "rows": frame.height,
            "selection_roles": sorted(
                str(value) for value in frame["selection_role"].unique()
            )
            if frame.height
            else [],
            "v11_mean_regret_uah": float(v11_mean_regret)
            if v11_mean_regret is not None
            else None,
            "strategy_kind": REGRET_SURROGATE_CANDIDATE_VALUE_V11_STRICT_LP_STRATEGY_KIND,
            "market_execution_enabled": False,
            "scope": "dfl_candidate_value_v11_strict_lp_gate_not_full_dfl",
            "not_market_execution": True,
        },
    )
    return frame
...
def dfl_v11_lava_dt_candidate_policy_frame(
    context,
    config: DflRegretSurrogateV1AssetConfig,
    dfl_lower_tail_risk_candidate_value_teacher_label_panel_v11_frame: pl.DataFrame,
) -> pl.DataFrame:
    """Train DT/LAVA-style candidate-index policy only when V11 labels are ready."""

    frame = build_dfl_v11_lava_dt_candidate_policy_frame(
        dfl_lower_tail_risk_candidate_value_teacher_label_panel_v11_frame,
        tenant_ids=_csv_values(config.tenant_ids_csv, field_name="tenant_ids_csv"),
        source_model_names=_forecast_model_names(config.source_model_names_csv),
        max_prior_neighbor_distance=config.max_prior_neighbor_distance,
    ...
    _add_metadata(
        ...
        {
            "rows": frame.height,
            "training_statuses": sorted(
                str(value) for value in frame["dt_lava_training_status"].unique()
            )
            if frame.height
            else [],
            ...
            "raw_hourly_action_imitation": False,
            "market_execution_enabled": False,
            "scope": "dfl_v11_lava_dt_candidate_policy_not_full_dfl",
            "not_market_execution": True,
        },
    )
    return frame
...
def dfl_v11_lava_dt_comparison_frame(
    context,
    dfl_lower_tail_risk_candidate_value_teacher_label_panel_v11_frame: pl.DataFrame,
    dfl_candidate_value_regret_surrogate_v11_frame: pl.DataFrame,
    dfl_v11_lava_dt_candidate_policy_frame: pl.DataFrame,
) -> pl.DataFrame:
    """Compare V11 selector, behavior cloning, DT/LAVA, V2+, and strict control."""

    frame = build_dfl_v11_lava_dt_comparison_frame(
        dfl_lower_tail_risk_candidate_value_teacher_label_panel_v11_frame,
        dfl_candidate_value_regret_surrogate_v11_frame,
    ...
    _add_metadata(
        context,
        {
            ...
            "dt_lava_rows": frame.filter(
                pl.col("selection_role") == REGRET_SURROGATE_V11_LAVA_DT_SELECTION_ROLE
            ).height
            if frame.height
            else 0,
            "behavior_cloning_role": REGRET_SURROGATE_V11_BEHAVIOR_CLONING_SELECTION_ROLE,
...
def dfl_candidate_value_teacher_label_panel_v7_frame(
    context,
    config: DflRegretSurrogateV1AssetConfig,
    dfl_feasible_schedule_candidate_library_v7_frame: pl.DataFrame,
    dfl_v2_plus_opportunity_backfill_requirements_frame: pl.DataFrame,
) -> pl.DataFrame:
    """Attach V7 candidate-value labels with realized outcomes kept diagnostic."""

    frame = build_dfl_candidate_value_teacher_label_panel_v7_frame(
        dfl_feasible_schedule_candidate_library_v7_frame,
        dfl_v2_plus_opportunity_backfill_requirements_frame,
        material_switch_delta_uah=config.material_switch_delta_uah,
        max_prior_neighbor_distance=config.max_prior_neighbor_distance,
    ...
    _add_metadata(
        context,
        {
            "rows": frame.height,
            "material_safe_switch_rows": frame.filter(
                pl.col("label_v7_material_safe_switch")
            ).height
            if frame.height
            else 0,
            ...
            "raw_hourly_action_imitation": False,
            "market_execution_enabled": False,
            "scope": "dfl_candidate_value_teacher_v7_not_full_dfl",
            "not_market_execution": True,
        },
    )
    return frame
...
def dfl_candidate_value_regret_surrogate_v7_frame(
    context,
    config: DflRegretSurrogateV1AssetConfig,
    dfl_candidate_value_teacher_label_panel_v7_frame: pl.DataFrame,
) -> pl.DataFrame:
    """Select V7 candidates only when prior support clears the abstention gate."""

    frame = build_dfl_candidate_value_regret_surrogate_v7_frame(
    ...
    _add_metadata(
        context,
        {
            ...
            "raw_hourly_action_imitation": False,
            "market_execution_enabled": False,
            "scope": "dfl_candidate_value_regret_surrogate_v7_not_full_dfl",
            "not_market_execution": True,
        },
    )
    return frame
...
def dfl_candidate_value_v7_strict_lp_benchmark_frame(
    context,
    config: DflRegretSurrogateV1AssetConfig,
    dfl_candidate_value_teacher_label_panel_v7_frame: pl.DataFrame,
    dfl_candidate_value_regret_surrogate_v7_frame: pl.DataFrame,
) -> pl.DataFrame:
    """Strict-score V7 candidate-value selections against corrected V2+."""

    strict_frame = build_dfl_candidate_value_v7_strict_lp_benchmark_frame(
        dfl_candidate_value_teacher_label_panel_v7_frame,
    ...
    _add_metadata(
        ...
        {
            "rows": strict_frame.height,
            "strategy_kind": REGRET_SURROGATE_CANDIDATE_VALUE_V7_STRICT_LP_STRATEGY_KIND,
            "gate_decision": gate.decision,
            "gate_description": gate.description,
            "diagnostic_signal_passed": gate.metrics.get(
                "diagnostic_signal_passed", False
            ...
            "raw_hourly_action_imitation": False,
            "market_execution_enabled": False,
            "scope": "dfl_candidate_value_v7_strict_lp_gate_not_full_dfl",
            "not_market_execution": True,
        },
    )
    return strict_frame
...
def dfl_candidate_value_v7_rolling_robustness_frame(
    context,
    config: DflRegretSurrogateV1AssetConfig,
    dfl_feasible_schedule_candidate_library_v7_frame: pl.DataFrame,
) -> pl.DataFrame:
    """Rolling prior-only robustness for V7 candidate-value selection."""

    frame = build_dfl_candidate_value_v7_rolling_robustness_frame(
        dfl_feasible_schedule_candidate_library_v7_frame,
        tenant_ids=_csv_values(config.tenant_ids_csv, field_name="tenant_ids_csv"),
        source_model_names=_forecast_model_names(config.source_model_names_csv),
        validation_window_count=config.validation_window_count,
    ...
    _add_metadata(
        context,
        {
            ...
            "raw_hourly_action_imitation": False,
            "market_execution_enabled": False,
            "scope": "dfl_candidate_value_v7_rolling_robustness_not_full_dfl",
            "not_market_execution": True,
        },
    )
    return frame
...
def official_global_panel_poland_lag24_experimental_rolling_strict_lp_benchmark_frame(
    ...
    strict_frame = build_official_global_panel_poland_lag24_experimental_rolling_strict_lp_benchmark_frame(
        real_data_benchmark_silver_feature_frame,
        tenant_ids=_csv_values(config.tenant_ids_csv, field_name="tenant_ids_csv"),
        entsoe_poland_lagged_feature_candidate_frame=(
            entsoe_poland_lagged_feature_candidate_frame
...
def official_global_panel_poland_lag24_experimental_tft_horizon_quantile_calibration_frame(
    ...
    calibration_frame = build_official_global_panel_tft_horizon_quantile_calibration_frame(
        official_global_panel_poland_lag24_experimental_rolling_strict_lp_benchmark_frame,
        min_prior_anchors=config.min_prior_anchors,
        rolling_calibration_window_anchors=(config.rolling_calibration_window_anchors),
        source_model_names=(POLAND_LAG24_EXPERIMENTAL_TFT_MODEL_NAME,),
        corrected_model_names=(POLAND_LAG24_EXPERIMENTAL_TFT_CALIBRATED_MODEL_NAME,),
...
def official_global_panel_poland_lag24_experimental_horizon_calibrated_strict_lp_benchmark_frame(
    ...
    strict_frame = build_official_global_panel_poland_lag24_experimental_horizon_calibrated_strict_lp_benchmark_frame(
        official_global_panel_poland_lag24_experimental_rolling_strict_lp_benchmark_frame,
        official_global_panel_poland_lag24_experimental_nbeatsx_horizon_calibration_frame,
        official_global_panel_poland_lag24_experimental_tft_horizon_quantile_calibration_frame,
    )
...
def dfl_poland_lag24_prior_tail_risk_veto_frame(
    ...
    coverage_status = (
        "full_365_ready" if anchor_count >= 365 else "insufficient_for_365_anchor_claim"
    )
...
def dfl_poland_lag24_rolling_vs_frozen_v2_plus_gate_frame(
    ...
    _add_metadata(
        context,
        {
            ...
            "gate_statuses": gate_frame.select("rolling_gate_status")
            .to_series()
            .unique()
            .sort()
            .to_list(),
...
def dfl_poland_lag24_experimental_vs_v2_plus_comparison_frame(
    ...
    experimental_frame = dfl_poland_lag24_experimental_schedule_value_learner_v2_plus_strict_lp_benchmark_frame
    baseline_frame = dfl_official_global_panel_schedule_value_learner_v2_plus_strict_lp_benchmark_frame
    comparison_rows = _summarize_poland_lag24_vs_v2_plus(
        experimental_frame,
        baseline_frame,
    )
...
def dfl_tft_combined_v2_plus_strict_lp_benchmark_frame(
    ...
    _add_metadata(
        context,
        {
            ...
            "fallback_to_v2_plus_count": gate.metrics.get("fallback_to_v2_plus_count"),
            "market_execution_enabled": False,
...
def dfl_nbeatsx_tft_meta_selector_rolling_strict_lp_benchmark_frame(
    ...
    gate = evaluate_dfl_nbeatsx_tft_meta_selector_gate(
        ...
        combined_source_model_name=config.combined_source_model_name,
        min_validation_tenant_anchor_count=(
            config.min_validation_tenant_anchor_count * config.validation_window_count
        ),
        min_mean_regret_improvement_ratio_vs_v2_plus=(
            config.min_mean_regret_improvement_ratio_vs_v2_plus
        ),
...
def dfl_nbeatsx_tft_meta_selector_prior_rolling_robustness_frame(
    ...
    _add_metadata(
        ...
        {
            "rows": robustness_frame.height,
            "rolling_pass_count": robustness_frame.filter(pl.col("rolling_pass")).height
            if robustness_frame.height
            else 0,
            "validation_window_count": config.validation_window_count,
            "validation_anchor_count": config.validation_anchor_count,
...
def _ua_context_grid_event_signal_frame(
    ...
    tenant_ids = sorted(
        {
            str(row["tenant_id"])
            for row in dfl_ua_context_backfill_requirements_frame.iter_rows(named=True)
...
def _summarize_poland_lag24_vs_v2_plus(
    ...
    best_baseline_mean = min(float(row["mean_regret_uah"]) for row in baseline_summary)
