...
The study also demonstrates disciplined negative evidence. TFT, Poland features, raw Decision Transformer selection, and several candidate-value variants did not replace the fallback under their stored screens. The historical random forest remains an in-packet diagnostic despite a lower packet mean because its training candidates are exact timestamp-shifted copies of its evaluation candidates, all four changed profile rows occur on one delivery date, and it misses the declared 166.0 UAH cutoff. The post-defense temporal suite removes candidate-content overlap but finds no beneficial protocol: latest windows abstain, while three earlier-window protocols switch and increase regret.
...
The corrective temporal suite uses three seeds only as model-stability checks and adjacent windows as protocol sensitivity. Its 14 rows are not independent replications. Full-abstention rows have mechanically degenerate zero intervals. Calibrated window 3 has an interval entirely above zero, while the two sparse raw-source harms have zero lower bounds because switches concentrate on one or two dates. These patterns are negative retrospective evidence, not a precise estimate of future harm.
...
The nearest research step remains prospective validation of a redesigned frozen challenger under the unchanged V2+ fallback and oracle scorer. The current RF safe-switch should not be promoted or reused unchanged: the post-defense temporal suite contains no benefit and three harmful protocols. The suite is not a substitute for prospective evidence because it was designed after the defect was known. A future protocol should timestamp estimator identity, features, hyperparameters, risk rules, and thresholds before evaluation, then require a genuinely later period, multi-period coverage, profile-level reporting, stable gains after costs, and a documented default-switch policy. A zero-tail-loss requirement would need a meaningful prospective denominator. Terminal-SOC equality or value, chained SOC, and explicit non-simultaneous charge/discharge sensitivity should be added before interpreting daily evaluator value as repeatable arbitrage value.
